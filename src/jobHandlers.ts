...
const queueAgentSelfUpdate = async (
  ...
  const script = [
    ...
    '  if [ "$NODE_CURRENT_MAJOR" -lt 22 ]; then',
    '    curl -fsSL https://deb.nodesource.com/gpgkey/nodesource-repo.gpg.key -o "$TEMP_DIR/nodesource-repo.gpg.key"',
    '    mkdir -m 0700 "$TEMP_DIR/gnupg"',
    '    gpg --batch --yes --homedir "$TEMP_DIR/gnupg" --dearmor -o "$TEMP_DIR/nodesource.gpg" "$TEMP_DIR/nodesource-repo.gpg.key"',
    '    NODE_ARCH="$(dpkg --print-architecture)"',
    '    printf \'deb [arch=%s signed-by=/etc/apt/keyrings/nodesource.gpg] https://deb.nodesource.com/node_22.x nodistro main\\n\' "$NODE_ARCH" > "$TEMP_DIR/nodesource.list"',
    '    sudo install -d -m 0755 /etc/apt/keyrings',
    '    sudo install -m 0644 "$TEMP_DIR/nodesource.gpg" /etc/apt/keyrings/nodesource.gpg',
    '    sudo install -m 0644 "$TEMP_DIR/nodesource.list" /etc/apt/sources.list.d/nodesource.list',
    ...
    '  ENV_TMP="$TEMP_DIR/agent.env"',
    "  cat > \"$TEMP_DIR/update-env-version.js\" <<'EOF'" ,
    "const fs = require('fs');" ,
    "const [srcPath, version, outPath] = process.argv.slice(2);" ,
    "const prefix = 'AGENT_VERSION=';" ,
    "const content = fs.existsSync(srcPath) ? fs.readFileSync(srcPath, 'utf8') : '';" ,
    "const normalized = content.replace(/\\r\\n/g, '\\n');" ,
    "const lines = normalized === '' ? [] : normalized.split('\\n');" ,
    "const filtered = lines.filter((line, index, arr) => !(index === arr.length - 1 && line === ''));" ,
    "let found = false;" ,
    "const out = filtered.map((line) => {" ,
    "  if (line.startsWith(prefix)) { found = true; return `${prefix}${version}`; }" ,
    "  return line;" ,
    "});" ,
    "if (!found) out.push(`${prefix}${version}`);" ,
    "fs.writeFileSync(outPath, out.join('\\n') + '\\n');" ,
    "EOF" ,
    '  node "$TEMP_DIR/update-env-version.js" "$ENV_FILE" "$TARGET_VERSION" "$ENV_TMP"',
    '  sudo install -m 0660 -o root -g rectrix-agent "$ENV_TMP" "$ENV_FILE"',
    '} >> "$LOG_PATH" 2>&1',
].join('\n');
